<template>
  <div>
    ...
    <DataTable :value="formData" size="small" lazy paginator :rows="rows" :first="first"
      columnResizeMode="fit" v-model:filters="filters.data" ref="datatableRef" dataKey="id" :totalRecords="totalRecords"
      ...
      <template #header class="rounded-t-md">
        <div class="flex justify-between">
          ...
          <IconField iconPosition="left" class="w-48 md:w-96">
            <InputIcon size="small">
              <i class="pi pi-search pt-1" />
            </InputIcon>
            <InputText v-if="(filters.data?.global as DataTableFilterMetaData)?.value !== undefined" v-model="(filters.data.global as DataTableFilterMetaData).value" placeholder="Keyword Search" class="pl-10 font-normal"
              @input="globalFilter" size="small" />
          </IconField>
        </div>
      </template>
      <template #loading>
      ...
      <template #empty>
        <p class="flex justify-center items-center gap-2 text-primary-400 " v-if="!loading && !columnLoading">
          No forms found.
          <RouterLink :to="{ name: 'create-form' }">
            <Button label="Add form" icon="pi pi-plus" size="small" />
          </RouterLink>
        </p>
      </template>
      <div v-for="(col, key) of columns" :key="key.toString()">
        <Column selectionMode="multiple" :class="col.class" v-if="key.toString() === 'multiple'" :style="col.style"></Column>
        <Column :field="key.toString()" :dataType="col.dataType" :header="col.header" :sortable="col.sortable" :filterField="key.toString()"
...
import ActionMenu from '@/views/Form/Submission/List/ActionMenu.vue';
import TableFilter from '@/views/Form/Submission/List/TableFilter.vue';
